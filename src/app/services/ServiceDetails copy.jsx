...
// import aboutt from "../../assets/assignment-services.webp";
...
const ServiceDetails = () => {
  ...
  const formatRouteParam = (params) => {
    return params.replace(/-/g, " ").replace(/%20/g, " ");
  };
  console.log(location, "locations");
  // Filter FAQ based on the current service label
  const filteredFaq = faq.filter(
    (item) => item.label === pathnames[pathnames.length - 1]
  );

  const filteredOptions = options.filter(
    (service) =>
      service.label === formatRouteParam(pathnames[pathnames.length - 1])
  );
  ...
  const title = filteredOptions[0]?.seoTitle || "Default SEO Title";
  const desc = filteredOptions[0]?.seoDescription || "Default SEO Description";
  ...
  const bannerTitle =
    currentServiceOption?.bannerTitle ||
    `${formatRouteParam(pathnames[pathnames.length - 1])}`; // Assuming you have an aboutImage property in your options
  const bannerDesc =
  ...
  const aboutTitle = currentServiceOption?.aboutTitle || (
    <>
      ...
      <span className="capitalize">
        {formatRouteParam(pathnames[pathnames.length - 1])}
      </span>{" "}
  ...
  const howWorksTitle = currentServiceOption?.howWorksTitle || (
    <>How {formatRouteParam(pathnames[pathnames.length - 1])} works</>
  );

  // Filter options based on the current service label
  // const currentServiceOption = options.find(
  //   (option) =>
  //     option.label === formatRouteParam(pathnames[pathnames.length - 1])
  // );
  ...
  // console.log("currentServiceOption", currentServiceOption);
            ...
            itemListElement: [
              {
                ...
                position: 1,
                name: "Home",
                item: "https://assignment-guru.com/",
              },
              ...pathnames.map((path, index) => ({
                "@type": "ListItem",
                position: index + 2,
                name: formatRouteParam(path),
                item: `https://assignment-guru.com/${path}`,
              })),
            ],
          },
        ]}
          ...
          <div className="how-service-works-section relative mx-auto py-16 max-w-screen-xl px-6">
            ...
            <div className="flex flex-col gap-10 lg:gap-3 mt-8 lg:flex-row justify-center items-center">
              ...
              <div>
                ...
                />
                {/* <Image loading="eager" src={ar1} alt="ar1" /> */}
              </div>
            </div>
          </div>
          <div className="feature-section relative mx-auto py-16 max-w-screen-xl px-6">
            <h4 className="text-4xl  text-center font-semibold text-black capitalize">
              Assignment Guru guarantees these features in
              <span className="">
                {" "}
                {formatRouteParam(pathnames[pathnames.length - 1])}{" "}
              </span>
